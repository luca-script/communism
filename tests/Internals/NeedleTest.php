...
it('rewrites constants into the replacement literal pool', function (): void {
    ...
    $function = Zend::lookupMethod(NeedleFixture::class, 'greet');

    if ($function === null) {
        throw new RuntimeException('Fixture method was not found');
    }
...
it('rewrites a body whose instruction count changed', function (): void {
    ...
    try {
        Assembler::write($body->insertBefore($body->count() - 1, $nop), $function->op_array);
...
it('replaces an ADD opcode with SUB', function (): void {
    ...
    $targetFunction = Zend::lookupMethod(NeedleFixture::class, 'add');

    if ($targetFunction === null) {
        throw new RuntimeException('Target method was not found');
    }
    ...
    try {
        $modified = $targetBody->replace($addIndex, $targetBody->instruction($addIndex)->withOpcode(Zend::opcodeId('SUB'), 'SUB', $subInstruction->handler));
        Assembler::write($modified, $targetFunction->op_array);

        expect((new NeedleFixture())->add(7, 3))->toBe(4);
    } finally {
        Assembler::write($targetBody, $targetFunction->op_array);
    }
});
...
it('does not invoke the rewriter when any injection fails to resolve', function (): void {
    ...
    expect(function () use ($body, $function, &$rewriterCalled): void {
        Injector::rewrite(
            $body,
            $function->op_array,
            [
