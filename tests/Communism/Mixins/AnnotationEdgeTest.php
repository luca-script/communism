...
describe('At', function (): void {
    ...
    it('validates At descriptions and action defaults', function (): void {
        ...
        ->and((new At('HEAD', action: 'BEFORE'))->action())->toBe('before')
        ->and((new At('HEAD', target: 'method', ordinal: 2, opcode: 'add'))->description())
        ->toBe('HEAD target method argument 2 opcode ADD')
        ->and((new At('INVOKE', target: ['method', ['numargs' => 1]]))->description())
        ->toBe('INVOKE target method')
    ...
    it('validates annotation constraints and mixin target matching', function (): void {
        ...
        expect(static fn(): Group => new Group(''))->toThrow(InvalidArgumentException::class)
            ...
            ->and(static fn(): Inject => new Inject('m', new At('HEAD'), variableType: 'int'))->toThrow(InvalidArgumentException::class)
            ->and(static fn(): Inject => new Inject('m', $at, constantType: 'invalid'))->toThrow(InvalidArgumentException::class)
            ->and(static fn(): Inject => new Inject('m', $at, nullValue: true, constantType: 'int'))->toThrow(InvalidArgumentException::class)
            ->and(static fn(): Inject => new Inject('m', $at, nullValue: true))->toThrow(InvalidArgumentException::class)
            ->and(static fn(): Inject => new Inject('m', $at, mode: 'invalid'))->toThrow(InvalidArgumentException::class)
            ->and((new Inject('m', new At('STORE'), variableType: 'int'))->variableType)->toBe('int')
            ->and((new Inject('m', new At('CONSTANT'), nullValue: true, constantType: 'null'))->nullValue)->toBeTrue()
